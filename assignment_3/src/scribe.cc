...
void Scribe::show_word_stats() {
	ScribeReporter reporter = ScribeReporter(get_file_contents());

	std::string output = reporter.formatted_words_counts_and_stars();

	std::cout << reporter.header() << output;
}
